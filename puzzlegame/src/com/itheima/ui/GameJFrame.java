...
public class GameJFrame extends JFrame implements KeyListener, ActionListener {
    ...
    public void getGameInfo(){
        //1.创建File对象表示存档文件夹
        File file = new File("D:\\JAVA_study\\java_study_code\\puzzlegame\\save");
        //2.获取存档文件夹的所有存档文件
        File[] files = file.listFiles();
        //3.遍历数组，获取每一个存档文件
        for (File f : files) {
            //4.获取存档文件中的步数，并将步数同步到item
            GameInfo gi;
            int step;
            try {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (obj == replayItem) {
            ...
            //获取配置文件中的路径，也就是值(双列集合中键account对应的值)
            //换图片时更改配置文件即可
           ...
           //存档，将有可能发生变化的数据存到本地文件
            ...
            //获取存档序号
            int index = str.charAt(2) - '0';//将该位置的字符转换成数字
            try {
                //根据存档号创建序列化流
                ...
                oos.flush();
                oos.close();
                //父类的actionPerformed没有抛出异常，子类不能抛出异常
            } catch (IOException ex) {
                ex.printStackTrace();
            }
            ...
            int index = str.charAt(2) - '0';//将该位置的字符转换成数字
            try {
                ObjectInputStream ois = new ObjectInputStream(new FileInputStream("D:\\JAVA_study\\java_study_code\\puzzlegame\\save\\save"+index+".data"));
                gi = (GameInfo) ois.readObject();
                //关流
                ois.close();
                //从本地文件获取数据
    ...
    private static void showJDialog(String path) {
        //创建一个弹框对象
